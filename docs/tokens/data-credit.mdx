...
DC can be delegated to other Helium Wallets but these must be IOT LNS provider Wallets in arrangement
with the LNS provider.
...
| Assert Location   | $10<sup>(</sup>[^1]<sup>, </sup>[^2]<sup>)</sup>|
| OUI               | $100                                            |
...
[^4]: [HIP-96](https://github.com/helium/HIP/blob/main/0096-wifi-ap-onboarding-structure.md) set an additional 
      MOBILE token onboarding fee for WiFi hotspots as well as the DC onboarding fee.
